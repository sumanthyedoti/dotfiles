...
# ---------- TLDR Update ---------------------------------------------
if command tldr -v tldr >/dev/null 2>&1; then
	tldr --update
	echo "tldr updated"
else
...
# ---------- Clear Calibre Cache ----------------------------------------
announce "Removing Calibre cache"
if [ -d "$HOME/.cache/calibre" ]; then
	calibre_cache_before=$(du -sh "$HOME/.cache/calibre" | cut -f1)
	echo "Before: $calibre_cache_before"
	if confirm "Clear Calibre cache now? [y/N]"; then
		rm -rf "$HOME/.cache/calibre"/*
	fi
	calibre_cache_after=$(du -sh "$HOME/.cache/calibre" | cut -f1)
	echo "After: $calibre_cache_after"
else
	echo "Calibre cache directory not found."
fi
